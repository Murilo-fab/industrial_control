...
from industrial_control.utils import (
    multiply_pose,
    rpy_from_quaternion,
    rad_to_deg_str,
...
class Interface(Node):
    ...
    def __init__(self, enable_moveit=True):
        ...
        # Meshes file path
        self.mesh_file_path = get_package_share_directory("industrial_control") + "/meshes/"
    ...
    def add_objects_to_planning_scene(self):
        package_share_directory = get_package_share_directory("industrial_control")
    ...
    def load_part_templates(self):
        ...
        self.sensor_template = cv2.imread(
            path.join(get_package_share_directory("industrial_control"), "resources", "sensor.png"), cv2.IMREAD_GRAYSCALE)
        self.regulator_template = cv2.imread(
            path.join(get_package_share_directory("industrial_control"), "resources", "regulator.png"), cv2.IMREAD_GRAYSCALE)
        self.battery_template = cv2.imread(
            path.join(get_package_share_directory("industrial_control"), "resources", "battery.png"), cv2.IMREAD_GRAYSCALE)
        self.pump_template = cv2.imread(
            path.join(get_package_share_directory("industrial_control"), "resources", "pump.png"), cv2.IMREAD_GRAYSCALE)
